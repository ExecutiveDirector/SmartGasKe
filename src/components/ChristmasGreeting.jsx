...
const ChristmasGreeting = () => {
  ...
  const [confetti, setConfetti] = useState([]); // New: Confetti pieces
  ...
  // Initial burst on first load – now with CONFETTI!
  useEffect(() => {
    // === Confetti Burst (Lots of colorful pieces falling and rotating) ===
    const colors = ['#ff0000', '#ff69b4', '#00ff00', '#ffff00', '#00ffff', '#ff00ff', '#ffd700', '#ff4500'];
    for (let i = 0; i < 120; i++) { // Lots of confetti!
      setTimeout(() => {
        const newConfetti = {
          id: Date.now() + i,
  ...
  useEffect(() => {
    if (audioRef.current) audioRef.current.onended = nextTrack;
  }, [currentTrack]);
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center relative overflow-hidden bg-gradient-to-br from-[#0a1a2f] via-[#1e3a5f] to-[#2c5282]">
      ...
      {/* Balloons, Fireworks, Snowflakes, Sparkles, etc. – unchanged */}
      {balloons.map(balloon => (
        <div
          key={balloon.id}
          className="absolute text-5xl pointer-events-none animate-rise-balloon"
          style={{
            left: `${balloon.left}%`,
            bottom: '-100px',
      ...
      >
        {/* All your beautiful card content remains exactly the same */}
        {/* ... (Header, messages, CTA, footer as before) ... */}
      ...
      <style jsx>{`
        ...
        /* Keep all your previous keyframes (fall, sway, rise-heart, starBurst, etc.) */
